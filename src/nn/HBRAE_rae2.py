...
class RecursiveAutoencoder_la(object):
    ...
    def forward_la(self, words_embedded ):
        ''' Forward pass of training recursive autoencoders using backpropagation
        through structures.  
    ...
    def backward_la(self, root_node, bad_root_node, total_grad, rec, sem, 
        ...
        top = True
        self.__backward_la( root_node, bad_root_node, total_grad, rec, sem, top, 
                yla_unnormalized, ylapla, bad_ylapla, optimal, delta_parent_out )
  
    def __backward_la( self, node, bad_node, total_grad, rec, sem, top, 
                yla_unnormalized, ylapla, bad_ylapla, optimal, delta_parent_out ):
        '''Backward pass of training recursive autoencoder using backpropagation
        through structures.
        ...
        if isinstance(node, InternalNode):
            ...
            if top:
                ...
                self.__backward_la(node.left_child, bad_node, total_grad, rec, sem, top, 
                ...
                self.__backward_la(node.right_child, bad_node, total_grad, rec, sem, top, 
...
if __name__ == '__main__':
    ...
    with Writer(output_file) as writer:     
        for i in xrange( len( src_instances ) ):
            ...
            src_root_node, src_rec_error = src_rae_la.forward_la( src_word_embedded )
            trg_root_node, trg_rec_error = trg_rae_la.forward_la( trg_word_embedded )
           ...
           # Calculate the similarity 
            ...
            src_internal_node_num = len(src_instance.words)-1
            trg_internal_node_num = len(trg_instance.words)-1
            if src_internal_node_num and trg_internal_node_num > 0:
                print '%20.8f, %20.8f, %18d' % (src_rec_error, src_rec_error / src_internal_node_num,
                        src_internal_node_num)
                print '%20.8f, %20.8f, %18d' % (trg_rec_error, trg_rec_error / trg_internal_node_num,
                        trg_internal_node_num)
            else:
                print '%20.8f, %20.8f, %18d' % (src_rec_error, src_rec_error, 0)
                print '%20.8f, %20.8f, %18d' % (trg_rec_error, trg_rec_error, 0)
