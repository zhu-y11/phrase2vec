...
class RecursiveAutoencoder_la(object):
    ...
    def backward_la(self, root_node, bad_root_node, total_grad, rec, sem, sem_o, 
                yla_unnormalized, ylapla, ylapla_o, bad_ylapla, bad_ylapla_o,
                optimal, optimal_o, delta_parent = None ):
        '''Backward pass of training recursive autoencoder using backpropagation
        through structures.
        ...
        top = True
        self.__backward_la( root_node, bad_root_node, total_grad, rec, sem, sem_o, top, 
                yla_unnormalized, ylapla, ylapla_o, bad_ylapla, bad_ylapla_o,
                optimal, optimal_o, delta_parent_out )
  
    def __backward_la(self, node, bad_node, total_grad, rec, sem, sem_o, top, 
                yla_unnormalized, ylapla, ylapla_o, bad_ylapla, bad_ylapla_o,
                optimal, optimal_o, delta_parent_out ):
        '''Backward pass of training recursive autoencoder using backpropagation
        through structures.
        ...
        if isinstance(node, InternalNode):
            ...
            if top:
                ...
                self.__backward_la(node.left_child, bad_node, total_grad, rec, sem, sem_o,
                top, yla_unnormalized, ylapla, ylapla_o, bad_ylapla, bad_ylapla_o,
                optimal, optimal_o, delta_parent_out_left )
      
                delta_parent_out_right = dot(self.Wi2.T, delta_parent)/rec - node.y2_minus_c2
                self.__backward_la(node.right_child, bad_node, total_grad, rec, sem, sem_o,
                top, yla_unnormalized, ylapla, ylapla_o, bad_ylapla, bad_ylapla_o,
                optimal, optimal_o, delta_parent_out_right )
            else:
                delta_sum = dot(self.Wo1.T, delta_out1)\
                        + dot(self.Wo2.T, delta_out2)\
                        + delta_parent_out
