...
from nn.HBRAE_rae1 import RecursiveAutoencoder
from nn.util import init_W
from nn.HBRAE_instance import Instance
from errors import GridentCheckingFailedError
from vec.HBRAE_wordvector import WordVectors
...
def compute_cost_and_grad(theta, instances, total_internal_node_num, word_vectors, embsize, 
                    lambda_reg, lambda_reg_L, Xidx, hiero_map):
...
def process( rae, word_vectors, instances, Xidx, hiero_map ):
    ...
    for instance in instances:
        # 是层次短语
        if False:#Xidx[0] in instance.words:
...
if __name__ == '__main__':
    ...
    if rank == 0: 
        ...
        # 每隔every步就存储一次模型参数
        callback = ThetaSaver( model + '_' + tp, every )    
        ...
        compute_cost_and_grad(theta, instances, total_internal_node, word_vectors, embsize,
                        lambda_reg, lambda_reg_L, Xidx, hiero_map )
